...
struct PTZHello: Equatable, CustomStringConvertible {
    let sysVer: String
    let camVer: String
    let backVer: String
    let bootVer: String
    ...
    var description: String { "MPTZ 11 sysVer=\(sysVer) camVer=\(camVer) backVer=\(backVer) bootVer=\(bootVer) splVer=\(splVer) pkgVer=\(pkgVer)" }
}

struct PTZHelloState: PTZReadable {
    ...
    static func get(for variant: PTZNone = .init()) -> PTZRequest {
        return .init(name: name, message: .init((0x06, 0x77)))
    }
}
